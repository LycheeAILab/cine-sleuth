...
- Install release `1.0.4` from `LycheeAILab/cine-sleuth`.
...
powershell -ExecutionPolicy Bypass -File $script -Version "1.0.4"
```
...
Reload WorkBuddy or start a new conversation. Confirm `cine-sleuth` appears in installed Skills, run `scripts/lab_auth.py` to authorize through LycheeAILab, then run `scripts/doctor.py`. Setup succeeds only when doctor reports version `1.0.4`, `authenticated: true`, and `runtime_ready: true`.
