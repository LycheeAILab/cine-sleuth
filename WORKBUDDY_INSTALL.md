...
- Install release `2.0.0` from `LycheeAILab/cine-sleuth`.
...
Invoke-WebRequest "https://raw.githubusercontent.com/LycheeAILab/cine-sleuth/v2.0.0/install-workbuddy.ps1" -OutFile $script
...
```bash
script="$(mktemp)"
curl -fsSL "https://raw.githubusercontent.com/LycheeAILab/cine-sleuth/v2.0.0/install-workbuddy.sh" -o "$script"
sh "$script" "2.0.0"
```

## Verify in WorkBuddy

Reload WorkBuddy or start a new conversation. Confirm `cine-sleuth` appears in installed Skills, run `scripts/lab_auth.py` to authorize through LycheeAILab, then run `scripts/doctor.py`. Setup succeeds only when doctor reports version `2.0.0`, `authenticated: true`, and `runtime_ready: true`.
