...
- Install release `1.0.1` from `LycheeAILab/cine-sleuth`.
...
Reload WorkBuddy or start a new conversation. Confirm `cine-sleuth` appears in installed Skills, run `scripts/lab_auth.py` to authorize through LycheeAILab, then run `scripts/doctor.py`. Setup succeeds only when doctor reports version `1.0.1`, `authenticated: true`, and `runtime_ready: true`.
