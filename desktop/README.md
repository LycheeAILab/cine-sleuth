# 镜探 Windows 桌面端 · 2.0.10

## 2.0.10 · 修复部分横屏视频无法分片

- 修复部分横屏素材缩放后产生奇数宽度，导致 H.264 代理片段编码失败的问题。
- 代理片段现在始终使用偶数宽高；切片时长、码率、清晰度和云端分析流程不变。
- 已使用触发问题的抖音素材完成链接解析、下载与 `1264×708 → 540×302` 转码验证。
...
本版采用 Lab 浅蓝白主题。左下角仅保留下载图标与版本号，点击展开更新浮层，可查看进度、重试和重启安装；点击空白或 Esc 关闭，不打断当前输入。桌面标签为 `desktop-v2.0.10`，与 Skill `v2.2.2` 独立。
...
1. 运行 [Windows x64 2.0.10 安装包](https://prod-lab-1321001571.cos.ap-guangzhou.myqcloud.com/releases/cine-sleuth/windows/x64/CineSleuth-2.0.10-Windows-x64-Setup.exe) 安装。
...
## 2.0.8：极速 HTML 整理与中文化

极速模式的 HTML 现在只保留逐镜表格，不再在表格下方重复全片正文和全局台词。嵌套花字、观察事实等对象会展开为可读文字，不再出现 `[object Object]`；新分析的说明性证据强制使用简体中文，常见英文镜头术语也会本地转为中文。任务切换增加请求隔离，打开和导出始终绑定当前结果；已有报告可直接重新生成极速表格。

## 2.0.9：旧英文证据的真正全文中文化

极速表格现在会检查逐镜字段的语言。中文证据仍完全在本地生成；对旧版任务中的整段英文 visuals、BGM、音效、分析和提示词，使用已配置模型按每批 8 镜做一次结构化快速翻译，不生成扩展分析。已完成批次立即缓存，中断后续传和再次导出都不重复调用。
