...
- 只需要表格时可选择“极速表格（推荐）”：直接复用已经完成的逐镜证据，不调用总结模型；“完整分析报告”仍保留分批长文生成流程。
...
本版采用 Lab 浅蓝白主题。左下角仅保留下载图标与版本号，点击展开更新浮层，可查看进度、重试和重启安装；点击空白或 Esc 关闭，不打断当前输入。桌面标签为 `desktop-v2.0.6`，与 Skill `v2.2.0` 独立。
