...
- 模型分析完成后，结果页点击“生成图文报告”，使用已保存的硅基流动模型和用户 Key 整理逐镜报告，包含分析、视频生成提示词及每个 seg 的原片首帧。
...
本版采用 Lab 浅蓝白主题。左下角仅保留下载图标与版本号，点击展开更新浮层，可查看进度、重试和重启安装；点击空白或 Esc 关闭，不打断当前输入。桌面标签为 `desktop-v2.0.6`，与 Skill `v2.0.0` 独立。
