...
function taskRow(task,local){
  ...
  else if(local&&task.status!=='completed'){const resume=element('button',task.source?.url&&!task.jobId?'重试链接':'继续');resume.disabled=!!state.running;resume.onclick=()=>perform(async()=>{resume.disabled=true;try{await api.resume(task.id);await refresh();}finally{resume.disabled=!!state.running;}});actions.append(resume);}
  if(task.jobId){const result=element('button','查看结果');result.onclick=()=>perform(()=>showResult(task.jobId));actions.append(result);}
  const remove=element('button','移除');remove.title='仅从本机列表移除，保留云端历史和文件';remove.disabled=!!state.running||reportBusy;
  remove.onclick=()=>perform(async()=>{remove.disabled=true;try{if(await api.removeRecord({kind:local?'local':'cloud',id:local?task.id:task.jobId})){await refresh();if(view==='history')await history();notice('已从本机列表移除，云端历史和文件已保留');}}finally{remove.disabled=!!state.running||reportBusy;}});actions.append(remove);
...
function renderReport(value){
  ...
  $('report-status').textContent=value?`${value.title} · ${value.segments} 个镜头 · ${value.model} · 已保存`:'极速表格直接使用中文证据；旧任务若是英文证据，只做分批快速翻译并缓存。完整报告会生成额外分析，耗时更长。';
}
