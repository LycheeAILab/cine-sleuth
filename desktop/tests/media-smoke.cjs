...
(async()=>{await fs.mkdir(root,{recursive:true});const source=path.join(root,'示例视频.mp4');
  ...
  result=await run(worker,[],JSON.stringify({url:'https://example.com/invalid',outputDir:path.join(root,'invalid-link')}),env);assert.notEqual(result.code,0);assert.match(result.err,/Bilibili 或 YouTube/);
