...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});
 try{for(const width of [1220,980]){
  ...
  await page.locator('#nav-settings').click();assert.equal(await page.locator('.model-card').count(),5);
  ...
  await page.getByRole('button',{name:'MiniMax M2.5 MiniMax',exact:true}).click();assert.match(await page.locator('#model-help').innerText(),/2026-09-11/);
  await page.getByRole('button',{name:'GLM 5.3 智谱'}).click();await page.locator('#model-key').fill('test-key-not-real');await page.locator('#save-settings').click();
  assert.equal(await page.locator('#model-key').inputValue(),'');assert.equal(await page.evaluate(()=>window.calls.save[0].model),'zai-org/GLM-5.3');
  await page.locator('#load-models').click();assert.equal(await page.locator('#model-preset').inputValue(),'zai-org/GLM-5.3');
