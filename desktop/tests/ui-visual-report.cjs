...
try{for(const width of [1220,980]){
 ...
 await page.addInitScript(()=>{
  ...
  window.cine={onEvent:fn=>listener=fn,state:async()=>({user:{displayName:'测试账号'},tasks:[{id:'local',jobId:'job',fileName:'测试原片.mp4',status:'completed'}]}),updateState:async()=>({status:'disabled',currentVersion:'dev'}),results:async()=>({status:'completed',chunks:[]}),summaryRead:async()=>null,
   reportRead:async()=>saved,reportGenerate:async(id,mode)=>{count++;window.reportCalls.push(mode);listener({type:'report-progress',id,message:'正在提取首帧…'});await new Promise(r=>setTimeout(r,150));if(count===1)throw Error('原视频已移动，请重新选择原片');saved={title:'离线图文报告',segments:3,model:'本地极速模式'};return saved;},reportOpen:async()=>{window.reportCalls.push('open');return '已打开';},reportExport:async()=>{window.reportCalls.push('export');return '已导出';}};
 });
 await page.goto('file:///'+path.resolve(__dirname,'../src/index.html').replaceAll('\\','/'));
 await page.getByRole('button',{name:'查看结果'}).click();
 await page.locator('#generate-fast-report').click();await page.locator('#report-status').filter({hasText:'原视频已移动'}).waitFor();
 assert.equal(await page.locator('#generate-fast-report').isEnabled(),true);
 await page.locator('#generate-fast-report').click();await page.locator('#open-report').waitFor();
 ...
 assert.deepEqual(await page.evaluate(()=>window.reportCalls),['fast','fast','open','export']);
